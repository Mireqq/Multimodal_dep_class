...
#Paths
MODEL_PATH = "/workspace/8bllama/models"
DATA_PATH = "/workspace/8bllama/deepseek_llama_balanced_dataset.pt"
...
def evaluate(model, dataloader):
    ...
    # Compute ROC AUC Score More Robustly
    try:
        unique_labels = np.unique(all_labels)  # Check which labels exist in the dataset
        unique_preds = np.unique(all_predictions)  # Check which labels were actually predicted

        #  Ensure all classes are present in predictions
        if len(unique_preds) < len(unique_labels):
            print(f" Warning: Some classes were not predicted. Computed ROC AUC may not be valid.")
        
        #  Apply Smoothing to Avoid Numerical Instability
        epsilon = 1e-10  # Small constant to prevent zero probabilities
        y_probs_smoothed = np.clip(all_probs, epsilon, 1 - epsilon)

        # Compute ROC AUC if possible
        roc_auc = roc_auc_score(all_labels, y_probs_smoothed, multi_class='ovr', average='macro')
    except ValueError as e:
        print(f" ROC AUC Calculation Failed: {e}")
        roc_auc = None  # Set to None if it cannot be computed
